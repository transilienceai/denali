...
function GitHubConnectionDetail({ connection, busy, navigation, onPrepare, onCollect, onValidate, onDisable, onDelete }: { connection: Connection; busy: string | null; navigation: FilterNavigation; onPrepare: () => void; onCollect: () => void; onValidate: () => void; onDisable: () => void; onDelete: () => void }) {
  ...
  const preparing = busy === `launch:${connection.id}`;
  const validating = connection.validation_state === "running" || busy === `validate:${connection.id}`;
  const isShared = connection.credential_reference.type === "platform_shared_github";
  ...
  return <section className="panel connection-detail">
    ...
    <div className="connection-safeguards"><div><strong>Connection lifecycle</strong><span>{isShared ? "Disabling or deleting here changes only Denali’s binding. To revoke the installation for all apps, disable the shared GitHub connection above. Previously collected evidence remains." : "Disabling prevents further validation. Deleting removes Denali’s connection configuration and validation history only; uninstall the GitHub App separately in GitHub. Previously collected evidence remains."}</span>{connection.credential_reference.type === "platform_shared_github" && <code>Platform connection {connection.credential_reference.platform_connection_id}</code>}{credential?.installation_id && <code>Installation ID {credential.installation_id}</code>}{connection.configuration.installer && <code>Verified installer {connection.configuration.installer.login} · user ID {connection.configuration.installer.id}</code>}</div>{connection.lifecycle_state === "active" ? <button disabled={busy === `disable:${connection.id}`} onClick={onDisable}><Power /> Disable</button> : <button className="danger-action" disabled={busy === `delete:${connection.id}`} onClick={onDelete}><Trash2 /> Delete configuration</button>}</div>
  </section>;
}
